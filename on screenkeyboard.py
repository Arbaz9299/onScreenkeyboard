...
def select(value):
    if value == 'Space':
        ...
        for button in leftShiftButtons:
            ...
            def command(x=button): return select(x)
            if button != 'Space':
                ttk.Button(root, text=button, command=command,
                           width=10, ).grid(row=varRow, column=varColumn)
...
root.title('On-Screen Keyboard created by Arbaz Ali')
root.config(bg='whitesmoke')
root.resizable(0, 0)

titleLabel = Label(root, text='On-Screen Keyboard',
                   font=('arial', 20, 'bold'), bg='whitesmoke', fg='gray30')
titleLabel.grid(row=0, columnspan=15)

textarea = Text(root, font=('arial', 15, 'bold'), height=10,
                width=100, wrap='word', bd=8, relief=SUNKEN)
...
for button in buttons:
    ...
    if button == 'Space':
        ttk.Button(root, text=button, command=command, width=30,
                   ).grid(row=6, column=0, columnspan=14)
